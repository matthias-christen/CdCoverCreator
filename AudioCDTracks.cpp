...
CAudioCDTracks::CAudioCDTracks (RetrieveMode mode)
{
	CWaitCursor ();
...
CAudioCDTracks& CAudioCDTracks::operator= (CAudioCDTracks& t)
{
	...
	// add new items
	for (int i = 0; i < t.GetNumTracks (); i++)
...
bool CAudioCDTracks::GetCDDBInfo()
{
	bool bResult = true;
	...
	strUser.Replace (' ', '_');
	strHello.Format ("&hello=%s+%s+CdCoverCreator+2.20&proto=3", strUser, szHostName);
