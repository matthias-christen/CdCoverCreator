	...
	HWND hwndProgress, HWND hwndStatus, CString strMsg)
{
	...
	CBindStatusCallback2 callback (nMethod == 1 ? BINDVERB_GET : BINDVERB_POST, strHeaders, strData);
	...
	else
	{
		...
		if (hr != S_OK)
			AfxMessageBox ("Error while accessing " + strURL + "\nThe server is possibly down. Try again later.", MB_ICONERROR);
		else
		{
			do
			{
...
/*					if (hwndStatus != NULL)
	...
	catch(CInternetException *e)
	{
		...
		CString strMsg;
		strMsg.Format ("ERROR: %s / %s", strURL.GetBuffer (-1), szError);
...
CString CHttpRequest::URLEncode(CString str)
{
	CString strEnc;
	CString s;
	char c;
