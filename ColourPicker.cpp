...
CColourPicker::CColourPicker()
{
    ...
    SetTextColour(CLR_DEFAULT /*GetSysColor(COLOR_BTNTEXT)*/);
...
//    CSize Margins(::GetSystemMetrics(SM_CXEDGE), ::GetSystemMetrics(SM_CYEDGE));
    ...
    pDC->DrawFrameControl(&m_ArrowRect, DFC_SCROLL, DFCS_FLAT | DFCS_SCROLLDOWN  | 
                          ((state & ODS_SELECTED) ? DFCS_PUSHED : 0) |
                          ((state & ODS_DISABLED) ? DFCS_INACTIVE : 0));
	pDC->Draw3dRect (&m_ArrowRect, ::GetSysColor (COLOR_3DHILIGHT), ::GetSysColor (COLOR_3DSHADOW));

//    pDC->DrawEdge(rect, EDGE_SUNKEN, BF_RECT);
	pDC->DrawEdge (rect, BDR_SUNKENOUTER, BF_RECT);	// MMC
...
//            pDC->SetTextColor((m_crColourText == CLR_DEFAULT)? 0 : m_crColourText);
			...
			// MMC 27.02.03
			if (m_crColourText == CLR_DEFAULT)
			{
				pDC->SetTextColor ((0.3 * GetRValue (m_crColourBk) + 0.59 * GetGValue (m_crColourBk) + 0.11 * GetBValue (m_crColourBk) <=
					((0.3 * 255.0) + (0.59 * 255.0) + (0.11 * 255.0)) / 2.0) ?
					0xFFFFFF : 0x000000);
			}
			else
