...
int i;
	for(i=0;i<256;++i)
	{
	    Gamma[i]=(int)(255 * pow((long double)i/255,(long double) m_fGamma) + (double)0.5);
	}
}
...
{
CFile file;
	if(!file.Open(csFileName, CFile::modeCreate | CFile::modeCreate | CFile::typeBinary))
	{
		return FALSE;
	}
	file.Close();
