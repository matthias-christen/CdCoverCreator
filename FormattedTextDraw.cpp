...
HRESULT CFormattedTextDraw::put_RTFText(BSTR newVal)
{
	...
	editStream.dwCookie = (DWORD) &m_editCookie;
...
HRESULT CFormattedTextDraw::Draw(void *hdcDraw, RECT *prc)
{
	...
	m_spTextServices->TxGetCachedSize (&dwWidth, &dwHeight);
...
HRESULT CFormattedTextDraw::get_NaturalExtent (HDC hDC, long* pWidth, long* pHeight)
{
	SIZEL szExtent;
	...
	szExtent.cx = *pWidth;
	szExtent.cy = *pHeight;
	m_spTextServices->TxGetNaturalSize (DVASPECT_CONTENT, hDC, NULL, NULL,
		TXTNS_FITTOCONTENT, &szExtent, pWidth, pHeight);

	return S_OK;
}

HRESULT CFormattedTextDraw::put_Height (long lHeight)
{
	m_lHeight = lHeight;
	return S_OK;
}
...
// Takes an HFONT and fills in a CHARFORMAT2W structure with the corresponding info
{
	...
	hDC = GetDC(hWnd);
	pCF->yHeight = -lf.lfHeight * LY_PER_INCH / GetDeviceCaps(hDC, LOGPIXELSY);
	ReleaseDC(hWnd, hDC);
