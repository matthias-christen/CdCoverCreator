...
public:
	...
	void TrackPopup (int nIndex, int nSubIndex, CPoint ptPos);
// Track popup menu (given the menu name)
	void TrackPopup(LPCSTR lpszName, CPoint ptPos);

	HMENU GetMenuHandle();

	inline CMenu* GetMenu () { return m_pMenu; }	// MCH 25.07.04
	inline CMenu* GetPopupMenu () { return m_pPopupMenu; }	// MCH 25.07.04
