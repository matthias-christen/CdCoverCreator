...
BEGIN_MESSAGE_MAP(CShellTreeView, CTreeCtrl)
...
*
****************************************************************************/
void CShellTreeView::PopulateTree() 
{
...
*
****************************************************************************/
void CShellTreeView::FillTreeView(LPSHELLFOLDER lpsf, LPITEMIDLIST  lpifq, HTREEITEM     hParent)
{
...
*
****************************************************************************/
void CShellTreeView::GetNormalAndSelectedIcons(LPITEMIDLIST lpifq,
                               LPTV_ITEM lptvitem)
{
...
*
****************************************************************************/
int CALLBACK CShellTreeView::TreeViewCompareProc(LPARAM lparam1, 
...
*
****************************************************************************/
void CShellTreeView::OnFolderExpanding(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
*
****************************************************************************/
void CShellTreeView::GetContextMenu(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	POINT			pt;
	LPTVITEMDATA	lptvid;  //Long pointer to TreeView item data
...
*
****************************************************************************/
BOOL CShellTreeView::OnFolderSelected(NMHDR* pNMHDR, LRESULT* pResult, CString &szFolderPath) 
{
	// TODO: Add your control notification handler code here
	LPTVITEMDATA	lptvid;  //Long pointer to TreeView item data
	LPSHELLFOLDER	lpsf2=NULL;
			...
			if (SUCCEEDED(hr))
				{
					...
					NM_TREEVIEW* pnmtv = (NM_TREEVIEW*)pNMHDR;
					if (ItemHasChildren(pnmtv->itemNew.hItem) && !(pnmtv->itemNew.state & TVIS_EXPANDEDONCE))
					{
...
*
****************************************************************************/
void CShellTreeView::EnableImages()
{
...
*
****************************************************************************/
BOOL CShellTreeView::GetSelectedFolderPath(CString &szFolderPath)
{
	LPTVITEMDATA	lptvid;  //Long pointer to TreeView item data
	LPSHELLFOLDER	lpsf2=NULL;
	static char		szBuff[MAX_PATH];
...
/****************************************************************************
*
*	FUNCTION:	FindTreePidl(	HTREEITEM nextNode,
*								HTREEITEM& folderNode, 
*								LPLVITEMDATA lplvid, 
*								bool& valid)
*
*	PURPOSE:	A recursive function that searches the Tree for a pidl
*				All searches are relative at the moment, so it will
*				yeild negative results if you try to do a root search.
*				Conduct all searches by passing the currently selected
*				node only.
*
*	MISC:	Dependent on LPLVITEMDATA struct recieved from CShellList()
*
*	BACKGROUND:	When the user doubleclicks on a folder in the listview
*				this function is called to try and find that folder in the
*				treeview so we can synchronize the directory change.
*
****************************************************************************/
void CShellTreeView::FindTreePidl(	HTREEITEM nextNode,
									LPLVITEMDATA lplvid, 
									bool& valid)
{
	LPTVITEMDATA	lptvid;  //Long pointer to TreeView item data
...
/*
MISC UNUSED FUNCTION. PURPOSE IS TUNNEL THE TREEVIEW TO FIND A SPECIFIC
FOLDER, eg the one a user typed! INCOMPLETE 
void CShellTreeView::TunnelFillTree(LPSHELLFOLDER lpsf, LPITEMIDLIST  lpifq, HTREEITEM     hParent)
{
    TV_ITEM         tvi;                          // TreeView Item.
    TV_INSERTSTRUCT tvins;                        // TreeView Insert Struct.
    HTREEITEM       hPrev = NULL;                 // Previous Item Added.
            ...
            while (S_OK==lpe->Next(1, &lpi, &ulFetched))
            {
                ...
                if (ulAttrs & (SFGAO_HASSUBFOLDER | SFGAO_FOLDER))
                {
					...
					if (ulAttrs & SFGAO_FOLDER)
					{
						...
						hPrev = InsertItem(&tvins);
						if (ulAttrs & (SFGAO_HASSUBFOLDER | SFGAO_FOLDER))
							if (ulAttrs & SFGAO_FOLDER && !(ulAttrs & SFGAO_REMOVABLE ))
								if ((ulAttrs & SFGAO_HASSUBFOLDER)){
									MessageBox(szBuff,"recurse");
									hr=lptvid->lpsfParent->BindToObject(lptvid->lpi,
											0, IID_IShellFolder,(LPVOID *)&lpsf2);

									if (SUCCEEDED(hr))
										TunnelFillTree(lpsf2,lptvid->lpifq,hPrev);
									else
										MessageBox("unable to bind");
								}
                   }
                   // Free this items task allocator.
                   lpMalloc->Free(lpifqThisItem);  
