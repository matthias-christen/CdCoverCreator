...
inline static bool strwnorm(TCHAR** s)
{
	...
	--n; //Start from the last string TCHAR.
	int i = n;
	for( ; (i > -1) && (*s)[i] > 0 && (*s)[i] < _T('!'); --i); //Find the first non-whitespace from the end.
...
static inline int strcmpwild_cset(const TCHAR** src,const TCHAR** dst)
{
	int find = 0;
	int excl = 0;
	int star = 1;
...
//</remarks>
static inline int strcmpwild(const TCHAR* src,const TCHAR* dst) 
{
	if(!src || !dst) return -1;
...
inline static bool load_file(const TCHAR* path,TCHAR** buffer,unsigned long* size,unsigned long tempsize = 4096)
{
	...
	*buffer = 0;
	...
	while(ReadFile(file_handle,(void*)temp,tempsize-sizeof(TCHAR),&read_bytes,0) && read_bytes && strcatgrow(buffer,temp))
	{
		*size += read_bytes;
		ZeroMemory(temp,sizeof(TCHAR)*tempsize);
	}
